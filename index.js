export default {
  install(Vue, config) {
    ...
    function dragMouseDownFn(e, el, targetParentId) {
      e = e || window.event;
      const parentPointBox = targetParentId ? document.getElementById(targetParentId) : el.parentNode;
    ...
    function dragBodyMoveFn(e, el) {
      ...
      if (left < 0 || left > +el.dataset.maxX || top < 0 || top > +el.dataset.maxY) {
        ...
        if (left > +el.dataset.maxX) {
          el.style.left = el.dataset.maxX + 'px';
        }
        if (top < 0) {
          el.style.top = '0px';
        }
        if (top > +el.dataset.maxY) {
          el.style.top = el.dataset.maxY + 'px';
        }
        // console.log('边界范围里了');
        // 取消监听
        document.body.removeEventListener('mousemove', el.__dragBodyMoveFn);
